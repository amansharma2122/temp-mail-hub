    ...
    queryFn: async () => {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        const [suspensionsResult, profilesResult] = await Promise.all([
          api.db.query<{user_id: string}[]>('user_suspensions', {
            filter: { is_active: true },
            select: 'user_id'
          }),
          api.db.query<{user_id: string, email_verified: boolean}[]>('profiles', {
            select: 'user_id, email_verified'
          })
        ]);

        const suspendedUserIds = new Set(
          suspensionsResult.data?.filter(s => userIds.includes(s.user_id)).map(s => s.user_id) || []
        );
        const emailVerifiedMap = new Map(
          profilesResult.data?.filter(p => userIds.includes(p.user_id)).map(p => [p.user_id, p.email_verified]) || []
        );
...
export const useAdminDomains = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.db.query("domains", {
...
export const useAdminBlogs = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await api.db.query("blogs", {
        order: { column: "created_at", ascending: false }
      });
...
export const useDomainMutations = () => {
  ...
  const addDomain = useMutation({
    mutationFn: async ({ name, isPremium }: { name: string; isPremium: boolean }) => {
      ...
      const { data, error } = await api.db.insert("domains", {
        name: domainName,
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      const { error } = await api.db.update("domains", 
        { is_active: !isActive },
        { id }
      );
  ...
  const deleteDomain = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await api.db.delete("domains", { id });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: adminQueryKeys.domains() });
...
export const useBlogMutations = () => {
  ...
  const deleteBlog = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await api.db.delete("blogs", { id });
      if (error) throw error;
    },
    onSuccess: () => {
...
export const useMailboxMutations = () => {
  ...
  const deleteMailbox = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await api.db.delete("mailboxes", { id });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: adminQueryKeys.mailboxes() });
